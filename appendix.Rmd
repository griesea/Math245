...
First, we check the residual plot of our linear model against the real interest rate. There seems to be some sort of positive parabolic relationship. However, after plotting the residuals against $IntRate^2$ we conclude there is no transformation to be done.
```{r,fig.height=4}
par(mfrow=c(1,2))
plot(resid(test.lm1) ~ (IntRate) , data = econ)
plot(resid(test.lm1) ~ I(IntRate^2) , data = econ)
par(mfrow=c(1,1))
```
...
```{r,fig.height=4}
test.lm2 <- lm(GDP_Rate ~ I(Inflation^2) + Inflation + IntRate + PrimEducRate +SecEducRate + TerEducRate + MCapRate+ Population + TradeRatio + Unemployment + S.P500 + Poverty +President + Senate + House, data = econ)
summary(test.lm2)
par(mfrow=c(1,2))
plot(resid(test.lm1) ~ (Inflation) , data = econ)
plot(resid(test.lm2) ~ Inflation, data = econ)
par(mfrow=c(1,1))
```
...
plot(resid(test.lm1) ~ log(PrimEducRate), data = econ)
par(mfrow=c(1,1))
```
...
plot(resid(test.lm1) ~ log(SecEducRate), data = econ)
par(mfrow=c(1,1))
```
...
plot(resid(test.lm1) ~ log(TerEducRate), data = econ)
par(mfrow=c(1,1))
```


Next, we check the residual plot of our linear model against MCapRate. We noticed a parabolic relationship previously and check for this. However, on the basis of the residual plot this seems to be inappropriate. As such, we check for a log transformation instead. However, this makes no improvement in the residual plot, so we proceed without a transformation.
```{r,fig.height=4}
par(mfrow=c(1,2))
plot(resid(test.lm1) ~ MCapRate, data = econ)
plot(resid(test.lm1) ~ log(MCapRate), data = econ)
par(mfrow=c(1,1))
```

Next, we check the residual plot of our linear model against Population. As expected, we conclude there is no transformation to be performed.
```{r,fig.height=4}
par(mfrow=c(1,2))
plot(resid(test.lm1) ~ Population, data = econ)
plot(resid(test.lm1) ~ log(Population), data = econ)
par(mfrow=c(1,1))
```
