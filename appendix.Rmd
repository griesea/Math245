...
par(mfrow=c(1,1))
plot(resid(econ1.lm)~YEAR, data=econ)
abline(h=0)
...
After removing the influential cases, we notice that the standard errors of the model have spiked causing many previously significant terms to become significant. Thus, we will to check if the large standard errors are due to collinearity. 
```{r}
summary(econ1.lm)
vif(econ1.lm)
scatterplotMatrix(~Inflation + IntRate + MCapRate, data = econ)
```
